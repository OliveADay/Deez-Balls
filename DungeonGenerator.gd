...
func _ready() -> void:
	...
	for i in rectAttempts:
		var rectpos = Vector2i(rng.randi_range(rrectMinsandMaxes[0],rectMinsandMaxes[1]), rng.randi_range(rectMinsandMaxes[0],rectMinsandMaxes[1]))
		...
		for b in range(rectTests.size())::
			...
			if rectTests[b].interswcts(recti):
